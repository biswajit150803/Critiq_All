...
export const StateProvider = ({ children }) => {
  ...
  const initializeWeb3 = useCallback(async () => {
    ...
    if (location.pathname != "/exchange" && networkId != 43113) {
    ...
    if (walletConnected) {
      try {
        const response = await axios.post(
          "https://critiqall-backend-1.onrender.com/api/users/login",
          {
            walletAddress,
          }
        );
  ...
  const userLogin = async () => {
    if (walletConnected) {
      try {
        const response = await axios.post(
          "https://critiqall-backend-1.onrender.com/customer/login",
          {
            walletAddress,
          }
        );
  ...
  const createSupervisor = async (number) => {
    ...
    try {
      const res1 = await axios.get(
        `https://critiqall-backend-1.onrender.com/getphone?phone=${number}`
      );
      ...
      if (res1.data.status === 0) {
        ...
        const sendData = await axios.post(
          "https://critiqall-backend-1.onrender.com/createphone",
          {
            sid: subscriberId,
            id: supervisorId,
