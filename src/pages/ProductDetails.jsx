...
const ProductDetails = () => {
  ...
  const fetchReviewsAndQuestions = useCallback(async () => {
    const questionsResponse = await axios.get(
      `https://critiqall-backend-1.onrender.com/api/form/question/${state.id}`
    );
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      setIsLoading(true);
      alert(
        "Since the LLM models are not yet deployed the AI validation is not working in live mode. Please check out the full working demo at : https://drive.google.com/file/d/12jvfVjfq8gJN4CdFLeehEtNWUrxWesP1/view?usp=drive_link"
      );
      console.log(JSON.stringify(newReview));
      const rev = JSON.stringify(newReview);
      const res = await axios.post(
        "https://critiqall-backend-1.onrender.com/sendmoney",
        {
          key: userData.pkey,
        }
      );
